...
public final class IdeaFiltersViewModel: ObservableObject {
    ...
    public init(
        ...
        self.dateFormatter = .server
        
        self.query = currentRequest.query ?? ""
        self.category = currentRequest.category ?? ""
        
        // Initialize dates in correct order
        self.createdAfterDate = filters.minDate
        self.createdBeforeDate = filters.maxDate
        
        // Then try to apply current filter values if they exist
        if let createdAfter = currentRequest.createdAfter,
           let date = dateFormatter.date(from: createdAfter) {
            self.createdAfterDate = date
        }
        
        if let createdBefore = currentRequest.createdBefore,
           let date = dateFormatter.date(from: createdBefore),
           date >= self.createdAfterDate {  // Ensure valid range
            self.createdBeforeDate = date
        }
    }
