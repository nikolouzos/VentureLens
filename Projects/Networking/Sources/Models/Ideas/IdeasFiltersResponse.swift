...
public struct IdeasFiltersResponse: Decodable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        minDate = Date(timeIntervalSince1970: TimeInterval(minDateTimestamp) / 1000)

        let maxDateTimestamp = try container.decode(Int.self, forKey: .maxDate)
        maxDate = Date(timeIntervalSince1970: TimeInterval(maxDateTimestamp) / 1000)
    }
}
